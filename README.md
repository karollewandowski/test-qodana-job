...
git branch -M main
git remote add origin https://github.com/karollewandowski/test-qodana-job.git
...
This plugin project declares an Qodana inspections profile: `.qodana/profiles/plugin.yaml`. It is referenced in the
...
To run Qodana code inspections locally, use:

```
./gradlew qodanaScan
```

It generates the report in `build/qodana/results/report/index.html`.
